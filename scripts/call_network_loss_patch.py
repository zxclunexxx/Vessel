...
# Newer call recovery layers may extend the disconnect helper signature, timing, and
# end-call reset block. Detect the behavior this patch owns instead of exact adjacency.
semantic_markers = [
    'let callDisconnectTimer = null;',
    'function clearCallDisconnectTimer()',
    'scheduleCallDisconnectCleanup(connection',
...
if all(marker in text for marker in semantic_markers):
...
function prepareCallConnection(user,peerId,video) {
  if (callConnection) return callConnection;
  callConnection=new RTCPeerConnection({iceServers:[{urls:'stun:stun.l.google.com:19302'}]});'''
if 'scheduleCallDisconnectCleanup(connection' not in text:
...
new_end = '''  callOffer=null;
  ...
  clearCallDisconnectTimer();
  if(callInviteTimer){clearTimeout(callInviteTimer);callInviteTimer=null;}'''
if 'clearCallDisconnectTimer();' not in text[text.find('async function endCall'):text.find('function toggleCallMicrophone')]:
